...
/**
 * Formulario para consulta de asistencia detallada
 */
class Application_Form_AsistenciaDetallada extends Fmo_Form_Abstract
{
        const E_FECHA_DESDE = 'fecha_desde';
        const E_FECHA_HASTA = 'fecha_hasta';
        const E_FICHA       = 'ficha';
        const E_GERENCIA    = 'gerencia';
        const E_DISPOSITIVO = 'dispositivo';
        const E_TIPO_PASE   = 'tipo_pase';
    ...
    public function init()
    {
        ...
        ->setLegend('Consulta Detallada de Asistencia')
        ...
        $dispositivo = new Zend_Form_Element_Select(self::E_DISPOSITIVO);
        $dispositivo->setLabel("Dispositivo:")
        ->addMultiOption("",'Seleccione...')
        ->addMultiOptions(Application_Model_DbTable_Sigrh_DispositivoFichaje::getPairsWithOrder('serial', new Zend_Db_Expr("descripcion || ' - ' || localidad"), NULL, array('descripcion','localidad')));
        $this->addElement($dispositivo);
